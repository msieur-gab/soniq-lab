...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .browse-btn { padding: 6px 10px; background: #14141f; border: 1px solid #2a2a3a; border-radius: 6px; cursor: pointer; font-size: 16px; line-height: 1; }
  .browse-btn:hover { border-color: #4444aa; }
...
<div class="ctrl">
  ...
  <label class="browse-btn" title="Select music folder">
    <input type="file" webkitdirectory directory multiple id="picker" onchange="pickFolder(this)" style="display:none" />
    &#128193;
  </label>
...
function pickFolder(input) {
  if (input.files.length > 0) {
    // webkitRelativePath gives "FolderName/subfolder/file.m4a"
    // We can't get the absolute path from the browser — show a hint
    const rel = input.files[0].webkitRelativePath;
    const topFolder = rel.split('/')[0];
    const el = document.getElementById('folder');
    if (!el.value) el.value = topFolder;
    alert('Browser security prevents reading the full path.\\nThe folder name is "' + topFolder + '".\\nPlease type or paste the full server path in the input field.');
  }
}
